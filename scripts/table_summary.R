...
hfi$pf_ss_women_inheritance_daughters[
  hfi$pf_ss_women_inheritance_daughters == "-"
] <- "0"
hfi$pf_ss_women_inheritance_widows[
  hfi$pf_ss_women_inheritance_widows == "-"
] <- "0"
...
# Result: Overall human freedom score across the countries is not showing
# noticeable improvement from 2008 to 2017, some had slightly improvement
...
# Result: In terms of the overall human freedom on average across the region in
# 2017, North America ranks the highest, followed by Western Europe, East Asia,
# Oceania, Eastern Europe, Latin America & the Caribbean, Caucasus & Central
# Asia, South Asia, Sub-Saharan Africa, Middle East & North Africa being the
# lowest.
...
# Result: In general, Western Europe & North America countries have higher
# personal freedom scores, with some of the lowest economic freedom scores
# while the others are included in both cases.
...
# Result: In terms of the safety and security aspect for women across the region
# in 2017, Sub-Saharan Africa ranks the highest, followed by Latin America & the
# Caribbean, Eastern Europe Western Europe, South Asia,
# Middle East & North Africa, East Asia, Caucasus & Central Asia, Oceania
...
avg_women_pf_2017 <- hfi %>%
  ...
  group_by(region) %>%
  summarise(
    avg_pf_ss_women_fgm = mean(as.numeric(pf_ss_women_fgm)),
    avg_pf_ss_women_inheritance = mean(as.numeric(pf_ss_women_inheritance)),
    avg_pf_ss_women_inheritance_daughters =
      mean(as.numeric(pf_ss_women_inheritance_daughters)),
    avg_pf_ss_women_inheritance_widows =
      mean(as.numeric(pf_ss_women_inheritance_widows)),
    avg_pf_movement_women = mean(as.numeric(pf_movement_women))
  ) %>%
  arrange(
    desc(
      avg_pf_ss_women_fgm,
      avg_pf_ss_women_inheritance,
      avg_pf_movement_women
    )
  )
...
# Result: In terms of the Women-Specific Personal Freedom Indicator across the
# region on average in 2017, Caucasus & Central Asia ranks the highest,
# followed by East Asia, Eastern Europe, Latin America & the Caribbean,
# North America, Oceania, South Asia, Western Europe,
