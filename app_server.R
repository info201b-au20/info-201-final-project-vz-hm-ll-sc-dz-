...
colnames(pf_identity_summary_by_region) <- c(
  "Year",
  "Region",
  "Legal",
  "Same_Sex",
...
cols_num <- col_names[col_names != "ISO_code" &
  col_names != "countries" &
  col_names != "year" &
  col_names != "region"]
hfi[cols_num] <- sapply(hfi[cols_num], as.numeric)
sapply(hfi, class)
...
server <- function(input, output) {
  ...
  output$scatter <- renderPlotly({
    title <- paste0(
      "Personal Freedom vs. Economic Freedom in ",
      input$scatter_year
    )
